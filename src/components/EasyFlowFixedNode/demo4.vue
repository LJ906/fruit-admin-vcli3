<template>
  <div v-if="easyFlowVisible">
    ...
    <!-- 编辑弹框 -->
    <el-dialog title=" " :visible.sync="dialogEdit" width="980px">
      <el-row :gutter="24">
        <el-col :span="6">
          <flow-node-menu treeType="approve" @typeMenuClick="typeMenuClick"></flow-node-menu>
        </el-col>
        <el-col :span="12">
          <flow-node-form ref="nodeForm" @saveSuccess="dialogEdit = false"></flow-node-form>
        </el-col>
        <el-col :span="6">
          <flow-node-menu treeType="apply" @typeMenuClick="typeMenuClick"></flow-node-menu>
        </el-col>
      </el-row>
    </el-dialog>
  </div>
...
import FlowNodeMenu from './components/node_menu_tree'
...
export default {
  ...
  methods: {
    ...
    // 添加树结构
    typeMenuClick(node, treeType) {
      console.log('typeMenuClick', node, treeType)
      this.$nextTick(_ => {
        this.$refs.nodeForm &&
          this.$refs.nodeForm.resetNodeName({
            name: node.name,
            type: treeType
          })
      })
